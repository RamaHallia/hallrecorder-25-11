...
Deno.serve(async (req)=>{
  ...
  try {
    const { transcript, summaryMode } = await req.json();
    const mode: "short" | "detailed" = summaryMode === "short" ? "short" : "detailed";
    ...
    const shortSummaryInstructions = `
MODE: RÉSUMÉ COURT
Objectif: conserver la même structure (Contexte, Thèmes, Décisions, Actions) mais avec un niveau de détail réduit.
Contrainte principale: chaque section ne doit contenir que 2 puces maximum, rédigées en une phrase directe (impacter les éléments clés uniquement).
`;

    const detailedSummaryInstructions = `
MODE: RÉSUMÉ DÉTAILLÉ
Objectif: appliquer la structure complète décrite dans les règles (3 à 4 puces par section, détails complets).
`;

    const modeInstructions = mode === "short" ? shortSummaryInstructions : detailedSummaryInstructions;
    ...
    const response = await fetch("https://api.openai.com/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          ...
          {
            role: "user",
            content: `${modeInstructions}\nTranscription:\n${transcript}\n\nRespecte STRICTEMENT le format JSON avec les champs "title" et "summary".`
          },
        ],
        temperature: 0,
    ...
    const title = parsed.title?.trim() || "Sans titre";
    const summary = parsed.summary?.trim() || "Aucun résumé généré";
    return new Response(JSON.stringify({
      title,
      summary
    }), {
 ...
 //     if (transcript.includes("Pas de données à transcrire") || transcript.includes("Pas de données") || transcript.trim().length < 50) {
 ...
 //     if (summary.includes("transcription ne contient pas de contenu pertinent") || summary.includes("transcription semble être incomplète") || summary.includes("ne permettant pas de générer") || summary.length < 100) {
